...
#### B-M2　A 撰寫台「一鍵前序完成」實際會標完「整個里程碑」 ✅ 已修（2026-07-10，客戶決策：全部上游）
- **原問題**：`incompletePrereqsOf` 只看任務自己的 dependencies、`prereqTodoTasks` 又抓「前序所在整個里程碑」的未完成任務 → 把選取任務自己(bbb)+子孫(a1~d1)都標 100%；且子任務無 deps → 選子孫不觸發。
- **修法（重定義「前序＝上游」）**：新增 `topOrder`(里程碑順序→頂層任務順序) + `topAncestorOf`；「前序」= 排在**選取任務頂層祖先之前**的所有未完成任務(跨里程碑)，不含自己群組/子孫/下游。
  - **一鍵標 100%** → 只標「全部上游」未完成(客戶選)；不再標整個里程碑、不標自己/子孫。
  - **觸發** → 點 bbb 或其**底下任何一層(6 層)** 都以頂層祖先判定 → 子孫一律會觸發前序提醒。
- 順帶：行內提示改里程碑摘要(上游可能很多筆)；`prereqJumpTarget` 取最接近的上游葉。
